...
import math
import re
from typing import Dict, List, Optional, Sequence, Set, Tuple, Pattern
...
IMPUTATION_RULES: Sequence[Tuple[Pattern[str], float, str]] = (
    (re.compile(r"injury_proxy_diff", re.IGNORECASE), 0.0, "injury proxy differential"),
    (re.compile(r"injury_proxy", re.IGNORECASE), 0.5, "injury proxy baseline"),
    (
        re.compile(r"schedule_pressure_index", re.IGNORECASE),
        0.5,
        "schedule pressure baseline",
    ),
    (
        re.compile(r"rest_stress_score", re.IGNORECASE),
        0.5,
        "rest stress baseline",
    ),
    (
        re.compile(r"travel_fatigue_score", re.IGNORECASE),
        0.5,
        "travel fatigue baseline",
    ),
    (
        re.compile(r"head_to_head_index", re.IGNORECASE),
        0.0,
        "head-to-head neutral",
    ),
    (
        re.compile(r"_diff(_season|_r\d+)$", re.IGNORECASE),
        0.0,
...
EXCLUDED_FINAL_COLUMNS: Sequence[str] = (
    ...
    "home_opp_points_season",
    "away_opp_points_season",
...
TEAM_ABBREVIATION_TOKEN_MAP: Dict[str, str] = {
    token: team_id
    for team_id, abbreviations in TEAM_ABBREVIATIONS.items()
    for token in (_normalize_team_token(abbrev) for abbrev in abbreviations)
    if token
}
...
TRAVEL_TIMEZONE_WEIGHT = 0.5
TRAVEL_ALTITUDE_NORMALIZER = 1000.0
TRAVEL_FATIGUE_RESPONSE = 0.8
...
AGGREGATE_SUFFIX_PATTERN = re.compile(r"(?:_season|_r\d+)$")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--max_season",
        default="2024-2025",
        help="Maximum season to include in YYYY-YYYY format (inclusive)",
    )
    parser.add_argument(
        "--windows",
        nargs="*",
...
def resolve_player_team_id(
    city: object, name: object, abbreviation: object = None
) -> Optional[str]:
    city_token = _normalize_team_token(city)
    name_token = _normalize_team_token(name)
    abbrev_token = _normalize_team_token(abbreviation)
    if abbrev_token:
        team_id = TEAM_ABBREVIATION_TOKEN_MAP.get(abbrev_token)
        if team_id:
            city_match = not city_token or city_token in TEAM_CITY_TOKENS_BY_TEAM.get(
                team_id, set()
            )
            name_tokens = TEAM_NAME_TOKENS_BY_TEAM.get(team_id, set())
            name_match = not name_token or (
                name_token in name_tokens
                or any(name_token and name_token in token for token in name_tokens)
            )
            if city_match and name_match:
                return team_id
            # Fall back to abbreviation-only resolution when metadata is inconsistent.
            return team_id
...
def integrate_injury_features(
    ...
    def _resolve_team_ids(subset: pd.DataFrame) -> pd.Series:
        ...
        abbreviations = subset.get("TeamAbbreviation")
        if cities is None and names is None and abbreviations is None:
        ...
        if abbreviations is None:
            abbreviations = pd.Series(np.nan, index=subset.index, dtype=object)
        city_values = cities.to_numpy(dtype=object, copy=False)
        name_values = names.to_numpy(dtype=object, copy=False)
        abbrev_values = abbreviations.to_numpy(dtype=object, copy=False)
        resolved = [
            resolve_player_team_id(city, name, abbrev)
            for city, name, abbrev in zip(city_values, name_values, abbrev_values)
        ]
    ...
    total_team_games = int(len(team_df))
    coverage_pct = (populated / total_team_games * 100.0) if total_team_games else 0.0
    logging.info(
        "Integrated injury proxy features from %s (populated %d team-games, %.1f%% coverage)",
        player_csv,
        populated,
        coverage_pct,
    )
    if total_team_games and coverage_pct < 40.0:
        logging.warning(
            "Injury proxy coverage is %.1f%% (%d of %d team-games); consider augmenting player-team mappings",
            coverage_pct,
            populated,
            total_team_games,
        )
...
def ensure_no_future_leakage(df: pd.DataFrame) -> None:
    """Ensure that aggregated columns are empty before a team plays any games."""

    required = {"SeasonStartYear", "GameDate_dt"}
    if not required.issubset(df.columns):
        logging.warning(
            "Skipping leakage validation because required columns are missing: %s",
    ...
    prefixes = (("HOME_", "HomeID"), ("AWAY_", "AwayID"))
    sorted_idx = df.sort_values("GameDate_dt").index
    prior_counts: Dict[str, pd.Series] = {}
    for prefix, team_col in prefixes:
        if team_col in df.columns:
            sorted_subset = df.loc[sorted_idx, [team_col, "SeasonStartYear"]]
            counts = (
                sorted_subset.groupby([team_col, "SeasonStartYear"], dropna=False)
                .cumcount()
                .astype(float)
            )
            aligned = counts.reindex(sorted_idx).reindex(df.index)
            prior_counts[prefix] = aligned.fillna(0).astype(int)
        else:
            prior_counts[prefix] = pd.Series(0, index=df.index, dtype=int)
    ...
    for prefix, team_col in prefixes:
        ...
        relevant_cols = [
            ...
            if col.startswith(prefix) and AGGREGATE_SUFFIX_PATTERN.search(col)
        ]
        if not relevant_cols:
            continue

        counts = prior_counts.get(prefix)
        if counts is None:
            continue

        for col in relevant_cols:
            suffix = AGGREGATE_SUFFIX_PATTERN.search(col)
            if not suffix:
                continue
            suffix_text = suffix.group(0)
            if suffix_text == "_season":
                min_required = 1
            else:
                try:
                    min_required = int(suffix_text[2:])
                except ValueError:
                    min_required = 1
            insufficient_history = counts < min_required
            invalid_idx = df.index[df[col].notna() & insufficient_history]
            if len(invalid_idx) > 0:
                df.loc[invalid_idx, col] = np.nan
                cleared_values += len(invalid_idx)
                affected_columns.add(col)

    home_counts = prior_counts.get("HOME_", pd.Series(0, index=df.index, dtype=int))
    away_counts = prior_counts.get("AWAY_", pd.Series(0, index=df.index, dtype=int))
    diff_columns = [
        col
        for col in df.columns
        if AGGREGATE_SUFFIX_PATTERN.search(col)
        and (
            col.startswith(SCHEDULE_DIFF_PREFIX)
            or col.startswith("Elo_diff_")
            or "_diff" in col
        )
    ]
    for col in diff_columns:
        ...
        if suffix_text == "_season":
            ...
            try:
                ...
                min_required = 1
        insufficient = (home_counts < min_required) | (away_counts < min_required)
        invalid_idx = df.index[df[col].notna() & insufficient]
        if len(invalid_idx) > 0:
            df.loc[invalid_idx, col] = np.nan
            cleared_values += len(invalid_idx)
            affected_columns.add(col)
    ...
    if cleared_values > 0:
        logging.warning(
            "Cleared %d aggregated values across %d columns due to insufficient prior games",
            cleared_values,
...
def validate_output(
    df: pd.DataFrame, windows: Sequence[int], min_start_year: int, max_start_year: int
) -> None:
    if (df["SeasonStartYear"] < min_start_year).any():
        raise ValueError("Rows prior to min_season detected after filtering")
    if (df["SeasonStartYear"] > max_start_year).any():
        raise ValueError("Rows beyond max_season detected after filtering")
    for metric in SELF_METRICS:
...
def impute_numeric_columns(df: pd.DataFrame) -> None:
    ...
    for col in df.columns:
        ...
        if not missing.any():
            continue
        for pattern, fill_value, label in IMPUTATION_RULES:
            if pattern.search(col):
                df.loc[missing, col] = fill_value
                count = int(missing.sum())
                pattern_summary[label] = pattern_summary.get(label, 0) + count
                touched_columns.add(col)
                break
    if pattern_summary:
        total_values = sum(pattern_summary.values())
        details = "; ".join(
            f"{label}: {count}" for label, count in sorted(pattern_summary.items())
        )
        logging.info(
            "Filled %d values across %d columns using domain-specific defaults (%s)",
            total_values,
            len(touched_columns),
            details,
        )
    ...
    if remaining:
        ...
        if len(remaining) > 10:
            preview += ", ..."
        logging.warning(
            "Numeric columns retain NaNs after imputation: %s", preview
        )
...
def main() -> None:
    ...
    max_start_year = season_start_year(args.max_season)
    if max_start_year < min_start_year:
        raise ValueError("max_season must be greater than or equal to min_season")
    before_filter = len(df)
    df = df[
        (df["SeasonStartYear"] >= min_start_year)
        & (df["SeasonStartYear"] <= max_start_year)
    ].copy()
    logging.info(
        "Filtered seasons outside %s-%s: kept %d of %d rows",
        args.min_season,
        args.max_season,
        len(df),
        before_filter,
    ...
    validate_output(df, windows, min_start_year, max_start_year)

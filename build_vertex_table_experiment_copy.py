...
DEFAULT_ROLL_WINDOWS: tuple[int, ...] = (4, 10, 20)
EWM_COMBINATION_SPAN = 5
EWM_COMBINATION_WEIGHT = 0.6
...
FINAL_ALLOWED_CONSTANT_COLUMNS: set[str] = {
    "HomeMarker",
    "RecentSeasonWeighting",
    "RecentGameWeighting",
}


def win_pct_to_elo(win_pct: pd.Series) -> pd.Series:
    """Approximate an Elo-style rating from historical win percentage."""

    win_pct = pd.to_numeric(win_pct, errors="coerce")
    if win_pct.empty:
        return pd.Series(dtype=float)

    clipped = win_pct.clip(lower=1e-6, upper=1 - 1e-6)
    with np.errstate(divide="ignore", invalid="ignore"):
        elo_delta = -400 * np.log10((1 / clipped) - 1)
    elo = 1500 + elo_delta
    elo[win_pct.isna()] = np.nan
    return elo


def select_metric_series(
    frame: pd.DataFrame, team_prefix: str, metric_slug: str
) -> pd.Series:
    """Pick the first available metric column for the desired stat."""

    candidates = [
        f"{team_prefix}_tm_{metric_slug}_r15_pre",
        f"{team_prefix}_tm_{metric_slug}_r5_pre",
        f"{team_prefix}_tm_{metric_slug}_pre",
    ]
...
def build_final_output_table(merged: pd.DataFrame) -> pd.DataFrame:
    """Project the merged feature set into the requested column layout."""

    merged = merged.copy()
    merged.reset_index(drop=True, inplace=True)

    home_win_pct = select_metric_series(merged, "home", "win_pct")
    away_win_pct = select_metric_series(merged, "away", "win_pct")

    home_self = {
        "HOME_SELF_Elo": win_pct_to_elo(home_win_pct),
        "HOME_SELF_EFG%": select_metric_series(merged, "home", "efg"),
        "HOME_SELF_TO%": select_metric_series(merged, "home", "tov_pct"),
        "HOME_SELF_OR%": select_metric_series(merged, "home", "oreb_pct"),
        "HOME_SELF_FTR": select_metric_series(merged, "home", "ft_rate"),
        "HOME_SELF_DR%": select_metric_series(merged, "home", "dreb_pct"),
    }
    home_opp = {
        "HOME_OPPONENT_EFG%": select_metric_series(merged, "home", "opp_efg_pct"),
        "HOME_OPPONENT_TO%": select_metric_series(merged, "home", "opp_tov_pct"),
        "HOME_OPPONENT_FTR": select_metric_series(merged, "home", "opp_ft_rate"),
    }
    away_self = {
        "AWAY_SELF_Elo": win_pct_to_elo(away_win_pct),
        "AWAY_SELF_EFG%": select_metric_series(merged, "away", "efg"),
        "AWAY_SELF_TO%": select_metric_series(merged, "away", "tov_pct"),
        "AWAY_SELF_OR%": select_metric_series(merged, "away", "oreb_pct"),
        "AWAY_SELF_FTR": select_metric_series(merged, "away", "ft_rate"),
        "AWAY_SELF_DR%": select_metric_series(merged, "away", "dreb_pct"),
    }
    away_opp = {
        "AWAY_OPPONENT_EFG%": select_metric_series(merged, "away", "opp_efg_pct"),
        "AWAY_OPPONENT_TO%": select_metric_series(merged, "away", "opp_tov_pct"),
        "AWAY_OPPONENT_FTR": select_metric_series(merged, "away", "opp_ft_rate"),
    }

    final = pd.DataFrame(
        {
            "GameID": merged.get("game_id"),
            "HomeID": merged.get("home_team_id"),
            "AwayID": merged.get("away_team_id"),
            "GameDate": merged.get("game_date"),
            "Location": merged.get("game_location", "unknown"),
            "HomeMarker": pd.Series(1, index=merged.index, dtype="Int64"),
            **home_self,
            **home_opp,
            **away_self,
            **away_opp,
            "Difference in the relative frequency of victories": home_win_pct
            - away_win_pct,
            "RecentSeasonWeighting": pd.Series(
                EWM_COMBINATION_WEIGHT, index=merged.index, dtype=float
            ),
            "RecentGameWeighting": pd.Series(
                1.0 - EWM_COMBINATION_WEIGHT, index=merged.index, dtype=float
            ),
            "HomeTeamWin": merged.get("label_home_win"),
        }
    )

    final["HomeID"] = final["HomeID"].astype("Int64")
    final["AwayID"] = final["AwayID"].astype("Int64")
    if "HomeTeamWin" in final.columns:
        final["HomeTeamWin"] = pd.to_numeric(final["HomeTeamWin"], errors="coerce").astype(
            "Int64"
        )
...
def drop_highly_correlated_pre_features(
    ...
    """Leave the pre-feature frame untouched to retain all requested metrics."""

    if pre_df.empty:
        return pre_df

    logging.info(
        "Skipping correlation-based pruning for prefeatures per latest requirements"
    )
    return pre_df


def drop_highly_correlated_final_features(
    df: pd.DataFrame, threshold: float = 0.995
) -> pd.DataFrame:
    """Return the final feature table unchanged (no correlation pruning)."""

    if df.empty:
        return df

    logging.info(
        "Skipping correlation-based pruning for final features per latest requirements"
    )
    return df
...
def build_game_rows(
    ...
    games_df["label_home_win"] = (games_df["home_margin"] > 0).astype("Int64")
    games_df["helper_home_win"] = games_df["label_home_win"].astype(float)
    ...
    ordered_cols = keep_cols + sorted(extra_cols) + ["label_home_win"]
    merged = merged[ordered_cols]
    merged = merged.sort_values(["game_date", "game_id"])

    final = build_final_output_table(merged)
    return final, helper_rate
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Rolling window sizes in games (experimental build enforces 5 and 15)",
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if "HomeTeamWin" not in out_df.columns:
        raise ValueError(
            "Output table must include HomeTeamWin as the predictive target"
        )
    extra_label_cols = [
        col for col in out_df.columns if col.startswith("label_") and col != "label_home_win"
    ]
    if extra_label_cols:
        logging.info(
            "Dropping helper label columns to keep Vertex target focused on home win: %s",
            extra_label_cols,
        )
    ...
    if len(out_df) > 1:
        ...
        const_cols = [
            ...
            and col not in FINAL_ALLOWED_CONSTANT_COLUMNS

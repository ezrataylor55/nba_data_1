...
def _load_training_module():
    ...
    try:
        return importlib.import_module("11year_basic")
    except ImportError as exc:  # pragma: no cover - defensive logging path
        raise FileNotFoundError(
            "Could not locate 11year_basic.py. Ensure it is present in the same "
            "directory or installable as a module before running this script."
        ) from exc
...
def build_prediction_columns(windows: Sequence[int]) -> List[str]:
    columns: List[str] = ["gameid", "homeid", "awayid", "gamedate", "location"]
    ...
    for (team_side, family), slug_prefix in side_prefixes.items():
        metrics = mvd.SELF_METRICS if family == "SELF" else mvd.OPP_METRICS
        for metric in metrics:
            metric_slug = METRIC_SLUGS[metric]
            for suffix in suffixes:
                columns.append(f"{slug_prefix}_{metric_slug}_{suffix}")

    for suffix in ("season",) + tuple(f"r{window}" for window in windows):
        columns.append(f"homeawaywinratedelta_{suffix}")

    columns.append("recencyweight")
    return columns
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Path to the input CSV containing team statistics. Defaults to "
            "TeamStatistics.csv in the current directory."
        ),
    )
    parser.add_argument(
        "--out_csv",
    ...
    parser.add_argument(
        "--schedule_csv",
        default="LeagueSchedule25_26.csv",
        help=(
            "Path to the league schedule CSV. Defaults to LeagueSchedule25_26.csv "
...
def resolve_schedule_aliases(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logging.info(
        "Resolved schedule aliases: %s",
        {canon: resolved[canon] for canon in sorted(resolved)},
    )
...
def load_schedule_for_date(path: str, snapshot_ts: pd.Timestamp) -> pd.DataFrame:
    ...
    schedule["GameDate"] = schedule["GameDate"].astype(str).str.slice(0, 10)
...
def build_game_rows(
    ...
    for _, sched_row in schedule.iterrows():
        ...
        for metric in mvd.SELF_METRICS:
            ...
            for suffix in suffixes:
                home_value = (
                    home_features.get(f"SELF_{metric}_{suffix}")
                    if home_features is not None
                    else np.nan
                )
                away_value = (
                    away_features.get(f"SELF_{metric}_{suffix}")
                    if away_features is not None
                    else np.nan
                )
                record[f"home_self_{metric_slug}_{suffix}"] = home_value
                record[f"away_self_{metric_slug}_{suffix}"] = away_value

        for metric in mvd.OPP_METRICS:
            metric_slug = METRIC_SLUGS[metric]
            for suffix in suffixes:
                home_value = (
                    home_features.get(f"OPP_{metric}_{suffix}")
                    if home_features is not None
                    else np.nan
                )
                away_value = (
                    away_features.get(f"OPP_{metric}_{suffix}")
                    if away_features is not None
                    else np.nan
                )
                record[f"home_opp_{metric_slug}_{suffix}"] = home_value
                record[f"away_opp_{metric_slug}_{suffix}"] = away_value

        for suffix in suffixes:
            home_val = (
                home_features.get(f"TeamWin_{suffix}")
                if home_features is not None
                else np.nan
            )
            away_val = (
                away_features.get(f"TeamWin_{suffix}")
                if away_features is not None
                else np.nan
            )
            if pd.notna(home_val) and pd.notna(away_val):
                record[f"homeawaywinratedelta_{suffix}"] = home_val - away_val
            else:
                record[f"homeawaywinratedelta_{suffix}"] = np.nan
    ...
    prediction_cols = build_prediction_columns(windows)
    for col in prediction_cols:
        if col not in output.columns:
            output[col] = np.nan
    output = output[prediction_cols]
...
def main() -> None:
    ...
    if tuple(windows) != DEFAULT_WINDOWS:
        raise ValueError(
            "This prediction export expects rolling windows of 4, 10, and 20 "
            "games to match the required column names."
        )
    stats_path = Path(args.in_csv)
    if not stats_path.exists():
        raise FileNotFoundError(
            f"Team statistics CSV not found at {stats_path.resolve()}. "
            "Pass --in_csv with the correct path (e.g., TeamStatistics.csv)."
        )
    schedule_path = Path(args.schedule_csv)
    if not schedule_path.exists():
        raise FileNotFoundError(
            f"Schedule CSV not found at {schedule_path.resolve()}. "
            "Pass --schedule_csv with the correct path (e.g., LeagueSchedule25_26.csv)."
        )
    ...
    schedule_df = load_schedule_for_date(str(schedule_path), snapshot_ts)
    logging.info(

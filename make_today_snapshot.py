...
def _load_training_module():
    ...
    script_path = Path(__file__).with_name("11year_basic_prediction_input.py")
    if not script_path.exists():
        raise FileNotFoundError(
            "Expected training script '11year_basic_prediction_input.py' alongside "
            "make_today_snapshot.py, but it was not found."
        )

    spec = importlib.util.spec_from_file_location("vertex_training", script_path)
    if spec is None or spec.loader is None:
        raise ImportError(
            "Unable to load feature helpers from 11year_basic_prediction_input.py"
        )

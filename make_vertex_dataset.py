...
def compute_recent_weightings(df: pd.DataFrame) -> None:
    ...
    seasons_ago = max_season_start - df["SeasonStartYear"]
    season_weight_raw = 1.0 / (1.0 + seasons_ago.clip(lower=0))
    season_weight_norm = _normalize_series(season_weight_raw)

    max_date = df["GameDate_dt"].max()
    days_since = (max_date - df["GameDate_dt"]).dt.days
    game_weight_raw = np.exp(-days_since / 730.0)
    game_weight_norm = _normalize_series(game_weight_raw)

    # Vertex AI expects weights to be a gentle preference rather than a dominant
    # signal. Anchor the weight near 0.9 and let the most recent season and game
    # contribute small positive adjustments so modern matchups receive the
    # highest emphasis without dwarfing historical rows.
    combined = 0.85 + 0.10 * season_weight_norm + 0.05 * game_weight_norm
    df["RecencyWeight"] = combined.clip(upper=1.0)

...
def drop_identical_feature_columns(
    df: pd.DataFrame, protected: Optional[Sequence[str]] = None
) -> None:
    """Remove columns whose values duplicate an earlier column."""
    ...
    for col in df.columns:
        ...
        series = df[col]
        if series.isna().any():
            series_for_hash = series.astype("object").copy()
            series_for_hash[series_for_hash.isna()] = sentinel
        else:
            series_for_hash = series
        signature = tuple(
            pd.util.hash_pandas_object(series_for_hash, index=False)
        )
        key = (signature, str(series.dtype))
        if key in seen_signatures:
            drop_cols.append(col)
        else:
            seen_signatures[key] = col
    if drop_cols:
        logging.info(
            "Dropping %d columns identical to earlier columns: %s", len(drop_cols), drop_cols
        )
...
def main() -> None:
    ...
    drop_identical_feature_columns(
        df,
        protected=(
            "GameID",
            "HomeID",

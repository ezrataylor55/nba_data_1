...
WIN_RATE_BASE = "HomeAwayWinRateDelta"
WIN_RATE_SUFFIX_MAP = {
...
def assign_features(
    ...
    for metric in SELF_METRICS:
        ...
        if home_base not in base_df.columns:
            base_df[home_base] = np.nan
        if away_base not in base_df.columns:
            base_df[away_base] = np.nan
        ...
        current_home = base_df.loc[home_features.index, home_base]
        current_away = base_df.loc[away_features.index, away_base]
        base_df.loc[home_features.index, home_base] = current_home.where(
            current_home.notna(), home_season_values
        )
        base_df.loc[away_features.index, away_base] = current_away.where(
            current_away.notna(), away_season_values
        )
        for window in windows:
...
def compute_recent_weightings(df: pd.DataFrame) -> None:
    ...
    season_weight = df["SeasonStartYear"].apply(
        lambda start: 1.0 / (1 + max(0, max_season_start - start))
    )
    max_date = df["GameDate_dt"].max()
    days_since = (max_date - df["GameDate_dt"]).dt.days
    game_weight = np.exp(-days_since / 365.0)
    combined = season_weight * game_weight
    if combined.max() > 0:
        combined = combined / combined.max()
    df["RecencyWeight"] = combined

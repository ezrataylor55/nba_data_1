...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if dated_sources:
        dated_ids = pd.Index(pd.unique(pd.concat(dated_sources, ignore_index=True)))
        if not dated_ids.empty:
            before = len(games_df)
            games_df = games_df.loc[games_df["game_id"].isin(dated_ids)].copy()
            removed = before - len(games_df)
            if removed:
                logging.info(
                    "Filtered %d rows lacking game_date across all sources", removed)

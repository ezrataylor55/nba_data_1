...
def load_table(path: str) -> pd.DataFrame:
    ...
    if "game_date" in df.columns:
        parsed_dates = pd.to_datetime(df["game_date"], errors="coerce", utc=True)
        if pd.api.types.is_datetime64tz_dtype(parsed_dates.dtype):
            try:
                df["game_date"] = parsed_dates.dt.tz_localize(None)
            except TypeError:
                df["game_date"] = parsed_dates.dt.tz_convert(None)
        else:
            df["game_date"] = parsed_dates
...
def clean_player_table(player_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "num_minutes" in player_df.columns:
        player_df["num_minutes_float"] = player_df["num_minutes"].map(minutes_to_float)
    else:
        player_df["num_minutes_float"] = np.nan
    if "win" in player_df.columns:
        player_df["win"] = player_df["win"].apply(map_binary_win_loss)
    return player_df
...
def aggregate_player_features(player_df: pd.DataFrame) -> pd.DataFrame:
    ...
    agg = player_df.groupby(group_cols, group_keys=False).apply(
        compute_player_group_features
    )
    agg = agg.reset_index()
    agg = agg.rename(
        columns={
...
def make_pre_features(
    ...
    for _, group in team_features.groupby(group_keys, sort=False):
        ...
        base = group[base_cols].copy()
        shifted = group[numeric_cols].shift(1)
        expanding = shifted.expanding(min_periods=1).mean()
        base["tm_games_played_pre"] = np.arange(len(group))
        rest_days = group["game_date"].diff().dt.days.astype(float)
        base["tm_rest_days_pre"] = rest_days
        ...
        for window, values in lookback_counts.items():
            base[f"tm_games_in_last_{window}_days_pre"] = values
        ...
        if win_series is not None:
            ...
            for result in wins:
                ...
                if pd.isna(result):
                    ...
                    current_loss += 1
                    current_win = 0
            base["tm_win_streak_pre"] = win_streak
            base["tm_loss_streak_pre"] = loss_streak
        ...
        if win_shifted is not None:
            ...
            for window in momentum_windows:
                ...
                base[f"tm_momentum_win_pct_r{window}_pre"] = rolling_win_pct
                base[f"tm_recent_wins_r{window}_pre"] = rolling_win_sum
                base[f"tm_recent_losses_r{window}_pre"] = rolling_loss_sum
        margin_shifted = shifted.get("margin")
        if margin_shifted is not None:
            margin_shifted = margin_shifted.astype(float)
            for window in momentum_windows:
                base[f"tm_momentum_margin_r{window}_pre"] = margin_shifted.rolling(
                    window=window, min_periods=1
                ).mean()
        if (
            "tm_momentum_margin_r3_pre" in base
            and "tm_momentum_margin_r10_pre" in base
        ):
            base["tm_momentum_margin_trend_pre"] = (
                base["tm_momentum_margin_r3_pre"] - base["tm_momentum_margin_r10_pre"]
            )
        for col in numeric_cols:
            base[f"tm_{col}_exp_pre"] = expanding[col]
        for window in roll_windows:
            rolling = shifted.rolling(window=window, min_periods=1).mean()
            for col in numeric_cols:
                base[f"tm_{col}_r{window}_pre"] = rolling[col]
        pre_frames.append(base)
...
def build_game_rows(
    ...
    for col in feature_cols:
        ...
        if (
            ...
            merged[f"delta_{base}"] = merged[col] - merged[other]
...
def limit_smoke(games_df: pd.DataFrame, team_df: pd.DataFrame, player_df: pd.DataFrame, smoke: int):
    ...
    games_df = ordered_games[ordered_games["game_id"].isin(keep_game_ids)]
    team_df = team_df[team_df["game_id"].isin(keep_game_ids)]
    player_df = player_df[player_df["game_id"].isin(keep_game_ids)]
    return games_df, team_df, player_df

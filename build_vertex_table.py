...
# A curated set of team-level box score metrics that are most predictive and
# interpretable for pre-game feature engineering. Restricting the rolling
# aggregates to this subset keeps the final table compact (hundreds of columns
# instead of thousands) while still covering Four Factors, scoring splits,
# momentum, and roster context derived from the original CSV inputs.
TEAM_CORE_NUMERIC_FEATURES: set[str] = {
    "win",
    "home",
    "season_wins",
    "season_losses",
...
def ensure_game_date(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if chosen_source:
        df.attrs["_game_date_source"] = chosen_source
    else:
        df.attrs["_game_date_source"] = None
    return df
...
def load_table(path: str) -> pd.DataFrame:
    ...
    source = df.attrs.get("_game_date_source")
    if source:
        logging.info("Detected game_date source column: %s", source)
    if "game_date" in df.columns:
        valid_dates = int(df["game_date"].notna().sum())
        logging.info(
            "Parsed %d/%d game_date values from %s",
            valid_dates,
            len(df),
            source or "game_date",
        )
...
def make_pre_features(
    ...
    for col in all_numeric_cols:
        if col in TEAM_CORE_NUMERIC_FEATURES:
    ...
    if not numeric_cols:
        logging.warning(
            "No numeric columns matched the curated whitelist; falling back to all numeric columns"
        )
        numeric_cols = all_numeric_cols
    else:
        dropped = sorted(set(all_numeric_cols) - set(numeric_cols))
        if dropped:
            logging.info(
                "Skipping %d numeric columns outside curated whitelist (e.g. %s)",
                len(dropped),
                ", ".join(dropped[:5]),
            )

...
def _build_team_lookup() -> Dict[Tuple[str, str], str]:
    ...
    for team_id, cities, names in TEAM_LOOKUP_CANDIDATES:
        ...
        normalized_names = {
            token for token in (_normalize_team_token(name) for name in names) if token
        }
...
TEAM_LOOKUP: Dict[Tuple[str, str], str] = _build_team_lookup()
...
def integrate_injury_features(
    ...
    players["TeamCity"] = players["TeamCity"].astype(str)
    players["TeamName"] = players["TeamName"].astype(str)

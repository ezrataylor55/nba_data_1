...
def assign_features(
    ...
    for feature in INJURY_FEATURES:
        for suffix in injury_suffixes:
            ...
            diff_col = f"{feature}_diff{suffix}" if suffix else f"{feature}_diff"
...
def integrate_injury_features(
    ...
    injury_cols = ["injury_proxy"] + [f"injury_proxy_r{w}" for w in INJURY_WINDOWS]
    ...
    required_fields = ["GameID", "TeamID", "Minutes", "PersonID"]
    ...
    missing_team = players["TeamID"].isna().sum()
    if missing_team:
        logging.warning(
            "Player statistics file %s has %d rows without team identifiers; they will be ignored",
            player_csv,
            int(missing_team),
        )
    players = players[players["TeamID"].notna()].copy()
    players = players[players["PersonID"].notna()].copy()
    if players.empty:
        logging.warning(
            "Player statistics rows with usable team and player identifiers were not found; skipping injury proxy features"
        )
        return
    ...
    def _aggregate_roster(group: pd.DataFrame) -> pd.Series:
        totals = (
            group.groupby("PersonID", as_index=False)["Minutes"].sum()
        )
        totals["PersonID"] = totals["PersonID"].astype(str)
        totals["Minutes"] = totals["Minutes"].astype(float)
        minutes_map = dict(zip(totals["PersonID"], totals["Minutes"]))
        minutes = totals["Minutes"].to_numpy(dtype=float)
        return pd.Series(
            {
                "player_minutes": minutes_map,
                "unique_players": len(minutes_map),
                "total_minutes": float(minutes.sum()),
            }
        )

    roster_stats = (
        players.groupby(["GameID", "TeamID"], sort=False).apply(_aggregate_roster).reset_index()
    )
    ...
    for indices in group_indices.values():
        prev_minutes: Optional[Dict[str, float]] = None
        prev_unique: Optional[int] = None
        for idx in indices:
            minutes_map = roster_stats.at[idx, "player_minutes"]
            total_minutes = roster_stats.at[idx, "total_minutes"]
            if (
                not isinstance(minutes_map, dict)
                or not minutes_map
                or not np.isfinite(total_minutes)
                or total_minutes <= 0
            ):
            ...
            if prev_minutes is None:
                ...
                for pid, mins in minutes_map.items():
                    shared_minutes += min(mins, float(prev_minutes.get(pid, 0.0)))
                returning_ratio = shared_minutes / total_minutes if total_minutes else np.nan
                if not np.isfinite(returning_ratio):
                    instability_values[idx] = np.nan
                else:
                    churn = max(0.0, min(1.0, 1.0 - returning_ratio))
                    current_unique = roster_stats.at[idx, "unique_players"]
                    if prev_unique and current_unique:
                        unique_diff = abs(current_unique - prev_unique) / max(
                            current_unique, prev_unique
                        )
                    else:
                        unique_diff = 0.0
                    instability = 0.7 * churn + 0.3 * unique_diff
                    instability_values[idx] = float(np.clip(instability, 0.0, 1.0))
            prev_minutes = minutes_map
            prev_unique = roster_stats.at[idx, "unique_players"]
    ...
    lookup = (
        roster_stats.set_index(["GameID", "TeamID"])
        .reindex(align_index)["instability_raw"]
    )

    team_df.loc[:, "_injury_instability"] = lookup.values

    grouped = team_df.groupby(["TeamID", "Season"], sort=False)
    team_df.loc[:, "injury_proxy"] = grouped["_injury_instability"].transform(
        lambda s: s.shift(1)
    )
    for window in INJURY_WINDOWS:
        col_name = f"injury_proxy_r{window}"
        team_df.loc[:, col_name] = grouped["_injury_instability"].transform(
            lambda s, w=window: s.shift(1).rolling(window=w, min_periods=w).mean()
        )

    team_df.drop(columns=["_injury_instability"], inplace=True, errors="ignore")

    populated = int(team_df["injury_proxy"].notna().sum())
    logging.info(

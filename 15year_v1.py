...
def integrate_injury_features(
    ...
    def _aggregate_roster(group: pd.DataFrame) -> pd.Series:
        totals = group.groupby("PersonID", as_index=False)["Minutes"].sum()
        totals["PersonID"] = totals["PersonID"].astype(str)
        minutes_values = totals["Minutes"].astype(float).to_numpy(dtype=float, copy=True)
        roster = frozenset(totals["PersonID"].tolist())

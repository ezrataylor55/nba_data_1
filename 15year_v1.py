...
INJURY_FEATURES: Sequence[str] = (
    "injury_proxy",
    "team_minutes_var",
    "unique_players_last5",
)
...
TEAM_LOOKUP_CANDIDATES: Sequence[Tuple[str, Sequence[str], Sequence[str]]] = (
    ...
    ("1610612740", ("new orleans", "nola"), ("pelicans", "pels")),
...
def compute_team_long_frame(
    ...
    for feature in list(REST_FEATURES) + list(TRAVEL_BASE_FEATURES) + list(INJURY_FEATURES):
...
def assign_features(
    ...
    for feature in INJURY_FEATURES:
        required_columns.extend([f"HOME_{feature}", f"AWAY_{feature}"])
        required_columns.extend([f"HOME_{feature}_season", f"AWAY_{feature}_season"])
        for window in windows:
            required_columns.extend(
                [f"HOME_{feature}_r{window}", f"AWAY_{feature}_r{window}"]
            )
    ...
    for feature in INJURY_FEATURES:
        if feature in home_features.columns:
            base_df.loc[home_features.index, f"HOME_{feature}"] = home_features[feature]
        if feature in away_features.columns:
            base_df.loc[away_features.index, f"AWAY_{feature}"] = away_features[feature]
        season_key = f"{feature}_season"
        if season_key in home_features.columns:
            base_df.loc[home_features.index, f"HOME_{season_key}"] = home_features[
                season_key
            ]
        if season_key in away_features.columns:
            base_df.loc[away_features.index, f"AWAY_{season_key}"] = away_features[
                season_key
            ]
        for window in windows:
            key = f"{feature}_r{window}"
            if key in home_features.columns:
                base_df.loc[home_features.index, f"HOME_{key}"] = home_features[key]
            if key in away_features.columns:
                base_df.loc[away_features.index, f"AWAY_{key}"] = away_features[key]
...
def integrate_injury_features(
    ...
    for feature in INJURY_FEATURES:
        ...
        required_columns.append(f"{feature}_season")
        for window in windows:
            required_columns.append(f"{feature}_r{window}")
    ensure_columns(team_df, required_columns)
    ...
    required_fields = ["GameID", "GameDate", "TeamCity", "TeamName", "Minutes", "PersonID"]
    ...
    players["GameDate"] = parse_game_date_series(players["GameDate"])
    valid_dates = players["GameDate"].notna().sum()
    if valid_dates == 0:
        logging.warning(
            "Player statistics file %s has no parsable game dates; skipping injury proxy features",
            player_csv,
        )
        return
    dropped_dates = len(players) - valid_dates
    if dropped_dates:
        logging.info(
            "Dropped %d player rows with invalid game dates while building injury features",
            dropped_dates,
        )
    players = players[players["GameDate"].notna()].copy()

    players["GameID"] = players["GameID"].astype(str)
    players["TeamCity"] = players["TeamCity"].astype(str).str.strip()
    players["TeamName"] = players["TeamName"].astype(str).str.strip()

    resolved_ids = [
        resolve_player_team_id(city, name)
        for city, name in zip(players["TeamCity"], players["TeamName"])
    ]
    players["TeamID"] = resolved_ids
    unmatched = players["TeamID"].isna().sum()
    if unmatched:
        logging.warning(
    ...
    if players.empty:
        logging.warning(
            "No player rows remained after team mapping; skipping injury proxy features"
        )
        return

    players["TeamID"] = players["TeamID"].astype(str)
    players["PersonID"] = players["PersonID"].astype(str)
    players["GameDate"] = players["GameDate"].apply(parse_game_date)

    roster_stats = (
        players.groupby(["GameID", "TeamID"], sort=False)
        .agg(
            team_total_minutes=(
                "Minutes", lambda s: float(s.sum()) if s.notna().any() else np.nan
            ),
            roster=(
                "PersonID",
                lambda s: frozenset(str(x) for x in s.dropna()),
            ),
        )
        .reset_index()
    )
    ...
    stats_lookup = roster_stats.set_index(["GameID", "TeamID"]).reindex(align_index)
    team_df["team_total_minutes"] = stats_lookup["team_total_minutes"].values
    team_df["player_roster"] = stats_lookup["roster"].values

    grouped = team_df.groupby(["TeamID", "Season"], sort=False)
    team_df["team_minutes_var_raw"] = grouped["team_total_minutes"].transform(
        lambda s: s.shift(1).rolling(window=5, min_periods=5).std(ddof=0)
    )

    unique_counts = grouped["player_roster"].apply(lambda s: rolling_unique_players(s, 5))
    unique_counts = unique_counts.reset_index(level=[0, 1], drop=True)
    team_df["unique_players_last5_raw"] = unique_counts

    team_df["team_minutes_var"] = grouped["team_minutes_var_raw"].transform(
        min_max_scale_series
    )
    team_df["unique_players_last5"] = grouped["unique_players_last5_raw"].transform(
        min_max_scale_series
    )

    minutes_norm = team_df["team_minutes_var"].to_numpy()
    unique_norm = team_df["unique_players_last5"].to_numpy()
    injury_proxy = np.full(len(team_df), np.nan)
    both_mask = ~np.isnan(minutes_norm) & ~np.isnan(unique_norm)
    injury_proxy[both_mask] = 0.5 * (minutes_norm[both_mask] + unique_norm[both_mask])
    minutes_only = ~np.isnan(minutes_norm) & np.isnan(unique_norm)
    injury_proxy[minutes_only] = minutes_norm[minutes_only]
    unique_only = np.isnan(minutes_norm) & ~np.isnan(unique_norm)
    injury_proxy[unique_only] = unique_norm[unique_only]
    team_df["injury_proxy"] = injury_proxy

    for feature in INJURY_FEATURES:
        if feature not in team_df.columns:
            continue
        team_df[f"{feature}_season"] = grouped[feature].transform(
            lambda s: s.shift(1).expanding().mean()
        )
        for window in windows:
            team_df[f"{feature}_r{window}"] = grouped[feature].transform(
                lambda s, w=window: s.shift(1).rolling(window=w, min_periods=w).mean()
            )
    ...
    drop_cols = [
        "team_total_minutes",
        "player_roster",
        "team_minutes_var_raw",
        "unique_players_last5_raw",
    ]
    existing = [col for col in drop_cols if col in team_df.columns]
    if existing:
        team_df.drop(columns=existing, inplace=True)

    populated = int(np.count_nonzero(~np.isnan(injury_proxy)))
    logging.info(

    ...
    for idx in ordered_index:
        ...
        if pd.notna(home_score) and pd.notna(away_score):
            try:
                ...
                margin = 0.0
            if margin > 0:
                base_component = math.log(margin + 1.0) * (
                    2.0 / (abs(home_rating - away_rating) * 0.001 + 2.0)
                )
                margin_multiplier = min(
                    ELO_BLOWOUT_MARGIN_CAP,
                    max(0.85, 0.5 + 0.5 * base_component),
                )
...
def try_derive_rate_columns(df: pd.DataFrame) -> None:
    for side in TEAM_SIDE_PREFIXES:
        ...
        if net_col in df.columns and df[net_col].isna().all():
        ...
        if opp_net_col in df.columns and df[opp_net_col].isna().all():
...
def compute_team_long_frame(
    ...
    for feature in list(REST_FEATURES) + list(TRAVEL_FEATURES):
    ...
    for _, group in per_team:
        ...
        for flag in away_flags:
            if flag:
                ...
                running = 0
            away_streak.append(float(running))
...
def assign_features(
    ...
    for feature in TRAVEL_FEATURES:
        for suffix in travel_suffixes:
            source_col = feature if suffix == "" else f"{feature}{suffix}"

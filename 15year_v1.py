...
def compute_team_long_frame(
    ...
    ensure_columns(team_df, list(REST_FEATURES) + list(TRAVEL_BASE_FEATURES) + list(INJURY_FEATURES))
    ...
    matchup_keys = [team_df["TeamID"], team_df["OpponentID"], team_df["Season"]]
    games_played = team_df.groupby(matchup_keys, sort=False).cumcount().astype(float)
    wins_prior = (
        team_df["TeamWin"].fillna(0.0)
        .groupby(matchup_keys, sort=False)
        .cumsum()
        .shift(1)
        .fillna(0.0)
    )
    score_margin = (
        coerce_numeric(team_df["TeamScore"]) - coerce_numeric(team_df["OpponentScore"])
    ).fillna(0.0)
    point_diff_prior = (
        score_margin.groupby(matchup_keys, sort=False)
        .cumsum()
        .shift(1)
        .fillna(0.0)
    )
    ...
    for feature in TRAVEL_AGG_FEATURES:
        if feature not in team_df.columns:
            continue
        travel_results[f"{feature}_season"] = travel_grouped[feature].transform(
            lambda s: s.shift(1).expanding().mean()
        )
        for window in windows:
            travel_results[f"{feature}_r{window}"] = travel_grouped[feature].transform(
                lambda s, w=window: s.shift(1).rolling(window=w, min_periods=w).mean()
            )
    if travel_results:
        team_df = pd.concat(
...
def assign_features(
    ...
    for metric in PER_POSSESSION_METRICS:
        for suffix in poss_diff_suffixes:
            diff_col = f"{metric}_diff{suffix}" if suffix else f"{metric}_diff"
            required_columns.append(diff_col)
    ...
    for metric in PER_POSSESSION_METRICS:
        home_col = f"HOME_SELF_{metric}"
        away_col = f"AWAY_SELF_{metric}"
        if home_col in base_df.columns and away_col in base_df.columns:
            base_df[f"{metric}_diff"] = coerce_numeric(base_df[home_col]) - coerce_numeric(
                base_df[away_col]
            )
...
def compute_win_rate_features(
    ...
    grouped = team_df.groupby(["TeamID", "Season"], sort=False)
    win_rate_results: Dict[str, pd.Series] = {
        "TeamWin_season": grouped["TeamWin"].transform(
            lambda s: s.shift(1).expanding().mean()
        )
    }
    for window in windows:
        win_rate_results[f"TeamWin_r{window}"] = grouped["TeamWin"].transform(
            lambda s, w=window: s.shift(1).rolling(window=w, min_periods=w).mean()
        )
...
def integrate_injury_features(
    ...
    for (game_id, team_id), group in players.groupby(["GameID", "TeamID"], sort=False):
        ...
        total_minutes = float(minutes_values.sum()) if minutes_values.size else np.nan
        dispersion = float(np.std(minutes_values)) if minutes_values.size else np.nan
        records.append(
            {
                "GameID": game_id,
                "TeamID": team_id,
    ...
    roster_stats["injury_proxy"] = roster_stats[f"injury_proxy_r{primary_window}"]
    ...
    for prefix in ("HOME", "AWAY"):
        for feature in INJURY_FEATURES:
            ...
            for window in INJURY_WINDOWS:
                columns.append(f"{prefix}_{feature}_r{window}")
    for feature in INJURY_FEATURES:
        columns.append(f"{feature}_diff")
        for window in INJURY_WINDOWS:
            columns.append(f"{feature}_diff_r{window}")
    poss_diff_suffixes = ["_season"] + [f"_r{window}" for window in windows]
    for metric in PER_POSSESSION_METRICS:
        columns.append(f"{metric}_diff")
        for suffix in poss_diff_suffixes:

...
def assign_features(
    ...
    for metric in SELF_METRICS:
        ...
        missing_cols = [
            col
            for col in (home_season_col, away_season_col)
            if col not in base_df.columns
        ]
        if missing_cols:
            base_df[missing_cols] = np.nan
        ...
        for window in windows:
            ...
            missing_cols = [
                col
                for col in (home_window_col, away_window_col)
                if col not in base_df.columns
            ]
            if missing_cols:
                base_df[missing_cols] = np.nan
    ...
    for feature in REST_FEATURES:
        if feature == "home_game_streak":
            home_col = "HOME_home_game_streak"
            if home_col not in base_df.columns:
                base_df[home_col] = np.nan
            if feature in home_features.columns:
                base_df.loc[home_features.index, home_col] = home_features[feature]
            continue
        if feature == "away_game_streak":
            away_col = "AWAY_away_game_streak"
            if away_col not in base_df.columns:
                base_df[away_col] = np.nan
            if feature in away_features.columns:
                base_df.loc[away_features.index, away_col] = away_features[feature]
            continue
        home_col = f"HOME_{feature}"
        away_col = f"AWAY_{feature}"
        missing_cols = [
            col for col in (home_col, away_col) if col not in base_df.columns
        ]
        if missing_cols:
            base_df[missing_cols] = np.nan
        if feature in home_features.columns:
    ...
    for feature in TRAVEL_AGG_FEATURES:
        for suffix in travel_suffixes:
            ...
            if source_col in home_features.columns:
                base_df.loc[home_features.index, home_col] = home_features[source_col]
            if source_col in away_features.columns:
                base_df.loc[away_features.index, away_col] = away_features[source_col]
    sos_suffixes = ["_season"] + [f"_r{window}" for window in windows]
    for suffix in sos_suffixes:
        source_col = f"strength_of_schedule{suffix}"
        if (
            source_col not in home_features.columns
            or source_col not in away_features.columns
        ):
            continue
        diff_col = f"{SCHEDULE_DIFF_PREFIX}{suffix}"
        if diff_col not in base_df.columns:
            base_df[diff_col] = np.nan
    ...
    if HEAD_TO_HEAD_COLUMN in home_features.columns and HEAD_TO_HEAD_COLUMN in away_features.columns:
        if HEAD_TO_HEAD_COLUMN not in base_df.columns:
            base_df[HEAD_TO_HEAD_COLUMN] = np.nan
        home_head = home_features[HEAD_TO_HEAD_COLUMN].fillna(0.0)
        away_head = away_features[HEAD_TO_HEAD_COLUMN].fillna(0.0)
        head_to_head_delta = home_head - away_head
        base_df.loc[home_features.index, HEAD_TO_HEAD_COLUMN] = head_to_head_delta
    if (
    ...
    for suffix in elo_suffixes:
        home_key = f"SELF_Elo_{suffix}"
        away_key = f"SELF_Elo_{suffix}"
        if home_key not in home_features.columns or away_key not in away_features.columns:
            continue
        diff_col = f"Elo_diff_{suffix}"
        if diff_col not in base_df.columns:
            base_df[diff_col] = np.nan
        diff_values = home_features[home_key] - away_features[away_key]
        base_df.loc[home_features.index, diff_col] = diff_values
    for suffix in WIN_RATE_SUFFIXES:
        source_col = f"TeamWin_{suffix}"
        if source_col in home_features.columns:
            home_target = f"HOME_team_win_rate_{suffix}"
            if home_target not in base_df.columns:
                base_df[home_target] = np.nan
            base_df.loc[home_features.index, home_target] = home_features[source_col]
        if source_col in away_features.columns:
            away_target = f"AWAY_team_win_rate_{suffix}"
            if away_target not in base_df.columns:
                base_df[away_target] = np.nan
            base_df.loc[away_features.index, away_target] = away_features[source_col]
    for suffix, col_name in WIN_RATE_DIFF_MAP.items():
        source_col = f"TeamWin_{suffix}"
        if source_col not in home_features.columns or source_col not in away_features.columns:
            continue

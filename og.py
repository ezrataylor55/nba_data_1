...
# Only a subset of numeric features receive exponentially weighted moving
# averages. This keeps the exported column count manageable while still
# providing the requested time-decay signals on core efficiency and momentum
# metrics. The names below map directly to columns produced from the original
# CSVs (team and aggregated player stats).
...
# Aggregated player momentum columns contain descriptive fragments (e.g.,
# ``ply_win_pct``). Matching on substrings keeps their exponentially weighted
# versions limited to momentum signals instead of every derived statistic.
TIME_DECAY_PLAYER_KEYWORDS: tuple[str, ...] = (
    "ply_minutes_avg",
    "ply_minutes_std",
    "ply_points_std",
    "ply_top_scorer_share",
    "ply_bench_points_share",
    "ply_plusminus",
    "ply_assist_ratio",
    "ply_off_reb_share",
    "ply_win_pct",
...
# A short exclusion list for numeric columns that behave like identifiers or
# create obvious redundancy once the matchup deltas are generated. Everything
# else derived from the original CSVs is allowed through the rolling feature
# builder so the exported table keeps as much predictive signal as possible.
TEAM_EXCLUDED_NUMERIC_FEATURES: set[str] = {
    "coach_id",
    "arena_id",
}

# Running cumulative sums are only meaningful for additive statistics. This set
# keeps the pre-game cumulative outputs focused on box-score values that can be
# aggregated across games without creating redundant rate features.
CUMULATIVE_FEATURE_BASES: set[str] = {
    "team_score",
    "opponent_score",
    "assists",
    "rebounds_total",
...
# When deriving lightweight predictive model features from season-to-date
# statistics, stay within a curated subset of efficiency and counting metrics so
# the outputs remain explainable and reproducible from the supplied CSVs.
MODEL_BASE_FEATURES: tuple[str, ...] = (
    "off_rtg",
    "def_rtg",
    "net_rtg",
    "efg",
...
MODEL_TARGET_MAP: dict[str, str] = {
    "margin": "tm_model_margin_pre",
    "team_score": "tm_model_team_score_pre",
    "opponent_score": "tm_model_opponent_score_pre",
}
...
def make_pre_features(
    ...
    for col in all_numeric_cols:
        if col in TEAM_EXCLUDED_NUMERIC_FEATURES:
            continue
        if any(col.startswith(prefix) for prefix in PLAYER_NUMERIC_PREFIXES):
            numeric_cols.append(col)
            continue
        numeric_cols.append(col)
    if not numeric_cols:
        logging.warning("No numeric columns available for rolling features")
        numeric_cols = all_numeric_cols

    observed_mask = (
        team_features[numeric_cols].notna().any() if numeric_cols else pd.Series(dtype=bool)
    )
    observed_cols = (
        observed_mask[observed_mask].index.tolist() if not observed_mask.empty else []
    )
    dropped_cols = sorted(set(numeric_cols) - set(observed_cols))
    if dropped_cols:
        logging.info(
    ...
    # Ensure rolling windows are unique and sorted for deterministic column
    # ordering.
    roll_windows = sorted({int(window) for window in roll_windows if int(window) > 0})

    pre_frames = []
    lookback_days = (3, 5, 7)
    season_origin: float | None = None
    if "season" in team_features.columns:
        all_seasons = pd.to_numeric(
            team_features["season"].str.split("-", n=1).str[0], errors="coerce"
        )
        if all_seasons.notna().any():
            season_origin = float(all_seasons.min())
    ...
    for _, group in team_features.groupby(group_keys, sort=False):
        ...
        if "season" in group.columns:
            season_series = group["season"].reset_index(drop=True)
            season_start_year = pd.to_numeric(
                season_series.str.split("-", n=1).str[0], errors="coerce"
            )
            season_features = {
                "tm_season_start_year_pre": season_start_year,
            }
            if season_start_year.notna().any():
                if season_origin is not None:
                    season_features["tm_season_recency_weight_pre"] = season_start_year - season_origin
                else:
                    season_features["tm_season_recency_weight_pre"] = season_start_year
            frames.append(pd.DataFrame(season_features, index=base.index))
        ...
        momentum_windows = sorted(
            set(int(window) for window in roll_windows if window > 0).union({3, 5, 10, 20})
        )
        ...
        cumulative_candidates = [
            col for col in numeric_cols if col in CUMULATIVE_FEATURE_BASES
        ]
        if cumulative_candidates:
            cumulative = shifted[cumulative_candidates].fillna(0).cumsum()
            history_counts = (~shifted[cumulative_candidates].isna()).cumsum()
            cumulative = cumulative.where(history_counts > 0)
            cumulative_dict = {
                f"tm_{col}_cum_pre": cumulative[col]
                for col in cumulative_candidates
            }
            frames.append(pd.DataFrame(cumulative_dict, index=base.index))
        ...
        ewm_cols = []
        for col in numeric_cols:
            if col in TIME_DECAY_FEATURE_CANDIDATES:
                ewm_cols.append(col)
            elif any(keyword in col for keyword in TIME_DECAY_PLAYER_KEYWORDS):
                ewm_cols.append(col)
        if ewm_cols:
        ...
        model_features = [col for col in MODEL_BASE_FEATURES if col in numeric_cols]
        model_targets = {
            target: name
            for target, name in MODEL_TARGET_MAP.items()
            if target in numeric_cols
        }
        if model_features and model_targets:
            feature_matrix = expanding[model_features].to_numpy(dtype=float)
            model_frames: dict[str, list[float | np.nan]] = {}
            for target_col, output_name in model_targets.items():
                preds: list[float | np.nan] = []
                alpha = 5.0
                feature_dim = len(model_features)
                xtx = alpha * np.eye(feature_dim)
                xty = np.zeros(feature_dim)
                sample_count = 0
                target_values = numeric_group[target_col].to_numpy(dtype=float)
                for idx in range(len(group)):
                    x_vec = feature_matrix[idx]
                    if sample_count == 0 or np.isnan(x_vec).any():
                        preds.append(np.nan)
                    else:
                        beta = np.linalg.solve(xtx, xty)
                        preds.append(float(np.dot(x_vec, beta)))
                    y_val = target_values[idx]
                    if np.isnan(y_val) or np.isnan(x_vec).any():
                        continue
                    xtx += np.outer(x_vec, x_vec)
                    xty += x_vec * y_val
                    sample_count += 1
                model_frames[output_name] = preds
            if model_frames:
                frames.append(pd.DataFrame(model_frames, index=base.index))
...
def build_player_momentum_features(
    ...
    stat_cols = [
        col
        for col in stat_candidates
        if col in player_extended.columns and player_extended[col].notna().any()
    ]
    skipped_stats = sorted(set(stat_candidates) - set(stat_cols))
    if skipped_stats:
        logging.info(
            "Skipping %d player stat bases with no data: %s",
            len(skipped_stats),
            skipped_stats[:10],
        )

    momentum_windows = sorted(
        set(int(window) for window in roll_windows if int(window) > 0).union({3, 5, 10, 20})
    )
    ...
    drop_nan_cols = [
        ...
        if col not in {"game_id", "team_id"} and not team_agg[col].notna().any()
    ]
    if drop_nan_cols:
        logging.info(
            "Dropping %d player momentum columns with no data: %s",
            len(drop_nan_cols),
            drop_nan_cols[:10],
        )
        team_agg = team_agg.drop(columns=drop_nan_cols)
    return team_agg
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    roll_windows = sorted(
        set(int(x) for x in args.roll_windows if int(x) > 0).union({3, 10, 20})
    )

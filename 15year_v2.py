...
def ensure_no_future_leakage(df: pd.DataFrame) -> None:
    ...
    for prefix, team_col in prefixes:
        ...
        if not relevant_cols:
            continue
        grouped = df.groupby([team_col, "SeasonStartYear"], dropna=False)
        for (team_id, season_start), group in grouped:
            if group.empty:
                continue
            ordered = group.sort_values("GameDate_dt")
            first_row = ordered.iloc[0]
            populated = first_row[relevant_cols].dropna()
            if populated.empty:
                continue
            sample = populated.index.tolist()
            preview = ", ".join(sample[:5])
            if len(sample) > 5:
                preview += ", ..."
            team_label = str(team_id)
            season_label = (
                int(season_start) if pd.notna(season_start) else season_start
            )
            raise ValueError(
                f"Potential information leakage detected for team {team_label} "
                f"season {season_label}: {preview} populated before any prior games"
            )
